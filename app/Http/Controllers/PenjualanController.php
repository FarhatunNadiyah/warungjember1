...
{
    $query = Penjualan::with('pembeli', 'kasir', 'detailPenjualan');

    if (!empty($request->tgl_awal) && !empty($request->tgl_akhir)) {
        $query->whereBetween('tanggal_pesan', [$request->tgl_awal, $request->tgl_akhir]);
    }

    if (!empty($request->search)) {
        $search = $request->search;
        $query->where(function ($q) use ($search) {
            $q->whereHas('pembeli', function ($sub) use ($search) {
                $sub->where('nama', 'like', "%$search%");
            })->orWhereHas('kasir', function ($sub) use ($search) {
